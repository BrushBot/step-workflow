    ...
    var Elem = ObjParent.extend({
      ...
      write: function(content) {
        ...
        if ( this.getVal() ) {
          ...
          if ( 'textarea' == this.getTag() ) {
            stepSet.textAreas[idx] = this.getVal() ;
          }
        }
...
// StepElem
    ...
    var StepElem = Elem.extend({
      ...
      getCheck: function(whichCheck) {
        whichCheck = whichCheck.replace(/^step/,'') ;
        var step = stepSet.byId[this.id] ;
    ...
    var StepDataElem = StepElem.extend({
      ...
      write: function(content) {
        ...
        forEachIn( stepX.data, function( key1, val1 ) {
          var rowsX = 1 ;
          if (unDef( sConfig['dataTypes'] ) ) {
            sConfig['dataTypes'] = {} ;
          }
          if (isDef( sConfig['dataTypes'][key1] ) && isDef( sConfig['dataTypes'][key1]['rows'] )) {
          ...
          var subStepX = elemMaker({'step': thisElem.step, 'class1': 'stepTextElem', 'rows': rowsX, '_name': key1,
            'attribs': {'name': key1 + '_' + stepX.id},
          }) ;
          if (unDef( sConfig['dataTypes'][key1] ) || unDef( sConfig['dataTypes'][key1]['label'] )) {
            sConfig['dataTypes'][key1] = sConfig['dataTypes'][key1] || {} ;
            sConfig['dataTypes'][key1]['label'] = key1.charAt(0).toUpperCase() + key1.slice(1) + ': ' ;
          }
...
// StepTextLineElem
    ...
    var StepTextLineElem = StepElem.extend({
      construct: function(argObj) {
        ...
        StepElem.construct.call(this, argObj) ;
        var thisElem = this ;
        ['_name','rows',].forEach(function(el,i){
          if ( isDef( argObj[el] ) ) {
            thisElem[el] = argObj[el] ;
          }
        }) ;
        thisElem['class1'] += '_' + thisElem['_name'] ;
      },
      ...
      getAttribs: function() {
        var attribs = StepCheckElem.getAttribsParent.call(this) ;
        attribs.type = 'text' ;
        attribs.value = this.getValAttrib() ;
        return(attribs) ;
      },
      getValAttrib: function(step) {
        step = step || this.step ;
        orSet( step, 'data', {} ) ;
...
// StepTextAreaElem
    ...
    var StepTextAreaElem = StepElem.extend({
      construct: StepTextLineElem.construct,
      ...
      getAttribs: function() {
        ...
        attribs['rows'] = rowsX ;
        attribs['name'] = attribs['name'] || this['name'] ;
        return(attribs) ;
      },
      getVal: StepTextLineElem.getValAttrib,
    }) ;
...
function debugObj(obj) {
  ...
  keys1.forEach(function(el,i){
    msg = msg + "key " + el + " val q{"+obj[el]+"}.\n" ;
  }) ;

  return( msg ) ;
} ;
function decodeHTML (str1) {
  return str1.replace(/\046quot;/g, '"')
             .replace(/\046gt;/g, '>')
             .replace(/\046lt;/g, '<')
             .replace(/\046amp;/g, '&');
};
    ...
    if (unDef(step.num)) {
      if ( isDef(parentStep) ) {
        if (unDef(step._num)) {
          ...
          if (unDef(thisStepSet.nextNum[parentStep.id])) {
            thisStepSet.nextNum[parentStep.id] = 1 ;
          }
          //step._num = thisStepSet.nextNum[step.id] ;
          //thisStepSet.nextNum[step.id] += 1 ;
          step._num = thisStepSet.nextNum[parentStep.id] ;
          thisStepSet.nextNum[parentStep.id] += 1 ;
        }
        step.num = '' ;
